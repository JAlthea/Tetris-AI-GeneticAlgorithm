...
for (var nowFigure = 0; j < countBlockFigure; j++) {    //하나의 블록 당 최대 3번 회전
	...
	for (var index = 0; index < N; index++) {    //X 좌표의 위치를 결정한다.
		for (var i = 0; i < 4; i++) {    //X를 +index만큼 움직인다. (dx 갱신)
		...
		if (!isVaildArrayIndex_v()) {    //X를 움직였을 때, 배열의 범위를 벗어나면 원래대로 복구한다.
			dx--;
			for (var i = 0; i < 4; i++) {
		...
		var tmpY = 0;
		while (isVaildArrayIndex_v()) {    //Y를 +1씩 바닥이나 다른 블록에 닿을 때까지 움직인다. (dy 갱신)
			for (var i = 0; i < 4; i++) {
		...
		for (var i = 0; i < 4; i++)    //적합도 계산을 위해서 움직인 블록의 위치를 가상 필드에 저장한다.
		...
		if (maxFitnessValue < nowFitnessValue) {    //적합도 결과값이 최고치를 갱신하는 경우
			...
			for (var i = 0; i < 4; i++) {    //가장 높은 값을 이에 해당하는 블록을 저장한다.
	...
	for (var i = 0; i < 4; i++) {    //블록이 회전하기 충분한 위치로 이동시킨다.
		virtualPc[i, 0] += 3;
		virtualPc[i, 1] += 3;
		virtualPa[i, 0] = virtualPc[i, 0];
		virtualPa[i, 1] = virtualPc[i, 1];
	...
	for (var i = 0; i < 4; i++) {    //다음 모양으로 회전시킨다.
